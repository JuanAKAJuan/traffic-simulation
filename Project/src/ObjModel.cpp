...
void ObjModel::ReadFile(string fileName)
{
    ...
    //{
    //	cout << mtl.first << endl;
    //	cout << mtl.second.Ka[0] << ' ' << mtl.second.Ka[1] << ' ' <<
    // mtl.second.Ka[2] << endl; 	cout << mtl.second.Ns << endl;
    // }
    ...
    while (!objFile.eof()) {
        ...
        if (temp == "v") {
            ...
            } while (temp[0] != 'f' && temp[0] != '#' && temp[0] != 'v' && temp[0] != 's' && temp[0] != 'g' && temp[0] != 'u' && !objFile.eof());
...
{
    Vertex center = { (xmin + xmax) / 2, (ymin + ymax) / 2, (zmin + zmax) / 2 };
...
{
    Vector3 size = { xmax - xmin, ymax - ymin, zmax - zmin };
