...
public:
    ...
    /// Initialize values of redOn, redOff, yellowOn, yellowOff, greenOn, and
    /// greenOff.
    void setMaterials();
...
private:
    /// The material values for each signal on and off.  Note by default, private
    /// members are not shown in Doxygen, but it can be enabled by checking
    /// EXTRACT_PRIVATE tag.
